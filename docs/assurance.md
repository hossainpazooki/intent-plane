...
testing monorepo's quickstart recompute probe runs both twins over a live
feed and byte-compares their reports. The traps an independent implementation must
...
| Tri-state verdicts; unevaluable never passes (P4) | **enforced & pinned** | Closed scoring domain; every transport/decode/non-2xx error ⇒ `Unevaluable` (`core/internal/scoring/scorer.go`). Since 2026-08-20 that matrix has a 3xx row: the scorer client NEVER follows a redirect, because a 3xx sends the criterion evaluation to an origin that is not the configured scorer — and with 301/302/303 the POST is downgraded to a body-less GET, so the responder is not even told which criterion it is answering about. Measured before the fix: all five redirect codes produced `Pass`. It now fails closed through the existing non-2xx row (§2.4; the same defect class as the declarant's redirect rule, found the same day, in shipped code). The zero-configuration deployment authorizes nothing: no trust root ⇒ every spec unattested; no scorer ⇒ every criterion unevaluable. |
...
| Standard signing envelopes, independently verifiable | built · test-grade | DSSE-shaped envelopes (PAE v1, ed25519) verifiable by standard tooling. Key authority is test-grade and every signature says so (`key_authority: "test"`); production key management (R1) is the named blocker before this row reads "enforced". |
...
test-grade until production key authority lands (R1); workload identity (R2) is asserted; the feed read
...
(any root key revokes any spec) — a who-may-revoke theory is production key-authority (R1) scope.
