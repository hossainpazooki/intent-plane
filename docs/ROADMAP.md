...
findings. Fork F4's routing decision is recorded in `CONTRACT-INTERFACE.md`
§I.4 (doctrine ahead of demonstration — G5).
...
|---|---|---|
| `force_scores` is a wire-reachable, unguarded total scoring bypass (`cmd/server/main.go` handleIntents; sanctioned by CONTRACT-SCORER §S.0/§S.3, pinned by `TestForceScoresStillWins`) | documented test affordance; **no env/build/auth guard** | Qualifies "unevaluable never passes", "fail-closed twice", and "artifacts are the only crossings" in any production claim. Guarding it is a contract change — needs its own amendment. |
...
| Thinned-set blindness: the gate cannot see that a spec carries fewer criteria than its source document requires | structural (P1 not yet enforced) | Belongs to the ATLAS-side minimum-criteria/coverage invariant + the resolver-extraction slice. B2 closed the *vacuous* case only. |
...
3. CI wheel-lane job (Linux, pinned `TIC_ATLAS_DIR`; make `_wheel_lane()`
