"""Service fail-closed matrix (CONTRACT.md §2.4) via TestClient — no network.
...
from scorer.app import create_app
from scorer.facts import StaticFactSource
from scorer.resolver import NullResolver
...
def client(facts=None, resolver=None) -> TestClient:
    src = StaticFactSource({"alpha": 250.0}) if facts is None else facts
...
def test_happy_path_pass():
    ...
    assert r.status_code == 200
    assert r.json() == {"result": "PASS", "basis": "alpha=250.00 >= 100.00"}
...
def test_malformed_request_is_4xx():
    r = client().post("/ml/evaluate", json={"criterion": "alpha"})
...
def test_unknown_request_fields_are_ignored():
    # Forward compatibility (CONTRACT.md §2.4): unknown fields must not fail the request.
...
def test_default_facts_are_empty_fail_closed() -> None:
    """A scorer booted with no facts knows nothing: every criterion is
    UNEVALUABLE. Demo facts live in a sibling facts file, injected via
    SCORER_FACTS_JSON — never baked into core."""
    from fastapi.testclient import TestClient

    app = create_app()  # no facts argument
    client = TestClient(app)
    resp = client.post("/ml/evaluate", json=body())
    assert resp.status_code == 200
    assert resp.json()["result"] == "UNEVALUABLE"
