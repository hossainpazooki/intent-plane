"""Cross-language golden fixtures (CONTRACT.md §9).
...
Fixtures live in THIS repo (contract/scorer, two levels up); SCORER_CONTRACT_DIR
...
def contract_dir() -> Path:
    d = Path(os.environ.get("SCORER_CONTRACT_DIR", DEFAULT_CONTRACT_DIR))
    if not d.is_dir():
        pytest.skip(
            f"contract fixtures not found at {d} — set SCORER_CONTRACT_DIR or pair "
...
def test_request_fixtures_parse_as_eval_request():
    from scorer.models import EvalRequest

    d = contract_dir()
    names = sorted(p.name for p in d.glob("request-*.json"))
    assert len(names) == 5, f"expected the 5 CONTRACT.md §9 request fixtures, found {names}"
    for name in names:
