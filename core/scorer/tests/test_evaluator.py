"""Evaluator unit behavior (CONTRACT.md §8).
...
from scorer.evaluator import evaluate
from scorer.models import EvalRequest


def req(criterion: str = "alpha", threshold: float = 100.0) -> EvalRequest:
    return EvalRequest(
...
def test_fact_at_or_above_threshold_passes():
    ...
    assert resp.basis == "alpha=250.00 >= 100.00"
...
def test_deterministic_given_same_fact():
    ...
    assert a == b
    assert a.basis == "alpha=123.46 >= 100.00"  # fixed 2-decimal rendering
