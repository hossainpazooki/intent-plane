...
echo "[boot] building and starting the gate"
go build -o "$repo/bin/intent-gate" "$repo/core/cmd/server"
INTENT_SCORER_URL="http://127.0.0.1:$scorer_port/ml/evaluate" INTENT_DATA_DIR="$data_dir" INTENT_ADDR=":$gate_port" "$repo/bin/intent-gate" &
gate_pid=$!
...
probe() { # name file want_terminal want_reason_part why [reject_reason_part]
    reject=${6:-}
    resp=$(curl -fsS -X POST -H "Content-Type: application/json" --data-binary "@$here/probes/$2" "$gate_url/v2/intents")
    ok=$(printf '%s' "$resp" | "$venv_py" -c "import json,sys; r=json.load(sys.stdin); print('1' if r['terminal']=='$3' and ('$4'=='' or '$4' in r['reason']) and ('$reject'=='' or '$reject' not in r['reason']) else '0')")
...
echo "RESULT: $pass/6 probes passed"
[ "$fail" -eq 0 ]
