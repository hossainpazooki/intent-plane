...
// B3 / W1 vocabulary gate. The plane's four roles — declarant (declares
// intents), author (drafts, holds no keys), attester (human author of record),
// gate (deterministic, sole ACHIEVED authority) — are the normative actor
// vocabulary.
//
...
func TestRoleVocabularyPresent(t *testing.T) {
	root := repoRoot(t)
	for _, doc := range []string{"README.md", "CONTRACT-INTERFACE.md"} {
